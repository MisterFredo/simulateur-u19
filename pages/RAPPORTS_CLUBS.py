...
with col1:
    ...
    selected_championnat = st.selectbox("Championnat", ["Tous"] + championnats_disponibles)

with col2:
    selected_centre = st.selectbox("Centre de formation", ["Tous", "OUI", "NON"])
    selected_top400 = st.selectbox("Top 400 Europe", ["Tous", "OUI", "NON"])
    selected_club = st.selectbox("Club", ["Tous"] + clubs_disponibles)
...
if selected_club != "Tous":
    df = df[df["NOM_CLUB"] == selected_club]
